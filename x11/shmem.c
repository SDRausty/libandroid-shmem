...
#ifndef __i386__
static bool use_kernel_shm = false;
static bool kernel_shm_tested = false;
#endif
...
#ifndef __i386__
static bool check_kernel_shm() {
	if (kernel_shm_tested) {
		return use_kernel_shm;
	} else {
		kernel_shm_tested = true;
		pid_t pid = fork();

		// Run external program to check if kernel shm*() functions are
		// supported. Note that test program may be killed by seccomp.
		if (pid == -1) {
			use_kernel_shm = false;
		} else if (pid == 0) {
			execl(TERMUX_PREFIX "/libexec/shm_test", "shm_test", NULL);
		} else {
			int test_status;

			if (waitpid(pid, &test_status, 0) == -1) {
				use_kernel_shm = false;
			} else {
				if (WIFEXITED(test_status) && !WEXITSTATUS(test_status)) {
					use_kernel_shm = true;
				} else {
					use_kernel_shm = false;
				}
...
#ifndef __i386__
	if (check_kernel_shm()) {
		return syscall(__NR_shmget, key, size, flags);
	}
#endif
...
//
// Provide functions from https://developer.android.com/ndk/reference/group/memory
